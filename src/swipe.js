const Swipe = (container, options) => {
	...
	const slide = (to, slideSpeed) => {
		...
		move(index, width * direction, slideSpeed || duration);
		move(to, 0, slideSpeed || duration);
			...
			if (!isScrolling) {
				if (isValidSlide && !isPastBounds) {
					...
					if (direction) {
						...
						move(index, slidePos[index]-width, duration);
						move(circle(index+1), slidePos[circle(index+1)]-width, duration);
					...
					if (continuous) {
						...
						move(circle(index-1), -width, duration);
						move(index, 0, duration);
						move(circle(index+1), width, duration);

					} else {

						move(index-1, -width, duration);
						move(index, 0, duration);
						move(index+1, width, duration);
					}
